...
cohen.d(errorLevel_actual$prop_corrected[errorLevel_actual$errorLevel == 'dtop'& errorLevel_actual$day == 8],errorLevel_actual$prop_corrected[errorLevel_actual$errorLevel == 'cmid' & errorLevel_actual$day == 8],paired = TRUE)
...
errorLevel_actual$errorLevel_ordered = NA
errorLevel_actual$errorLevel_ordered[errorLevel_actual$errorLevel == 'dtop'] = 'a'
errorLevel_actual$errorLevel_ordered[errorLevel_actual$errorLevel == 'cmid'] = 'b'
errorLevel_actual$errorLevel_ordered[errorLevel_actual$errorLevel == 'bbottom'] = 'c'

errorLevel_actual_level_summary = summarySEwithin(errorLevel_actual, measurevar = 'prop_corrected',withinvars = c('day','errorLevel_ordered'), idvar = 'subject', na.rm = TRUE) %>% filter(!is.na(errorLevel_ordered))
ggplot(errorLevel_actual_level_summary, aes(x = errorLevel_ordered, y = prop_corrected, fill = errorLevel_ordered ))+geom_bar(stat = 'identity', position = 'dodge')+geom_errorbar(aes(ymin = prop_corrected-se,ymax =prop_corrected+se), position = position_dodge(.9), width = .2) + 
  geom_point(data = errorLevel_actual, aes(x = errorLevel_ordered, y = prop_corrected),
             size = 2, color = "black",shape = 21,position = position_jitter(width = 0.1), alpha = 1) + 
  facet_wrap(~day)+scale_y_continuous(expand = c(0,0), limits = c(0,.2))+
  theme(axis.line=element_line(),
        ...
        axis.ticks = element_line(colour = axisColor, size = tickWidth), 
        axis.ticks.length = unit(tickLength,"cm"))
ggsave('multiday_errorLevel_forced_POINTS.eps')
# by PT bin
...
ggplot(errorLevel_emlactual_level_summary, aes(x = eml_actual, y = prop_corrected, fill = errorLevel_ordered ))+geom_bar(stat = 'identity', position = 'dodge')+geom_errorbar(aes(ymin = prop_corrected-se,ymax =prop_corrected+se), position = position_dodge(.9), width = .2) + 
  geom_point(data = errorLevel_emlactual, aes(x = eml_actual, y = prop_corrected),
             position = position_dodge(0.9),  # Same dodge as the bars
             size = 2, color = "black",shape = 21) + 
  facet_wrap(~day)+scale_y_continuous(expand = c(0,0), limits = c(0,.25))+
  theme(axis.line=element_line(),
        panel.grid = element_blank(),
        panel.background = element_rect(fill = "white", colour = "black")
        ,panel.border = element_rect(fill = NA, colour = "white"),
...
#ggsave('multi_eml_errorLevel_nocorr_POINTS.eps')
```
...
ggplot(muVals_plot, aes(x = level, y = mu, fill = day))+geom_bar(stat = 'summary', fun.y= 'mean')+geom_point(data = muVals, aes(x = level, y = value),position = position_jitter(width = 0.1), alpha = 1)+
...
# ggsave('multiday_muvals_POINTS.eps')
