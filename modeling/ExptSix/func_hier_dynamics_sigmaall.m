...
function ll = func_hier_dynamics_sigmaall(params,sub_responses,sub_PTs)

% free parameters
mu = params(1:3); % the MEAN time each level is prepared [hand, couplet, finger]
sigma = params(4:6); % variance in time each level is prepared [hand, couplet, finger]
rho = params(7); % weight on uniform guessing preparation function
